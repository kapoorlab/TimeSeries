...
public class InteractiveChirpFit implements PlugIn {
    ...
    public void Card() {
       ...
       for (int i = 0; i < inputfiles.length; ++i) {
		...
		final Scrollbar FREQU = new Scrollbar(Scrollbar.HORIZONTAL, this.FrequInt, 1, MIN_SLIDER,
				MAX_SLIDER + 1);
		
		final Scrollbar CHIRP = new Scrollbar(Scrollbar.HORIZONTAL, this.ChirpInt, 1, MIN_SLIDER,
				MAX_SLIDER + 1);
    ...
    public void displayclicked(final int trackindex){
	 ...
	 timeseries = ExtractSeries.Normalize(ExtractSeries
				.gatherdata(inputfiles[trackindex]));
	...
	public void updateCHIRP(){
		...
		FunctionFitter chirp = new FunctionFitter(this, timeseries, UserChirpModel.Linear);
		chirp.checkInput();
